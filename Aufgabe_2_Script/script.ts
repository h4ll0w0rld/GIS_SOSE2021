namespace Aufgabe_A {
    // let zahlen: number[] = [3, 2, 9, 7, 2, 1];
    //console.log(smallestNumber(zahlen));


    // function smallestNumber(zahlen: number[]) {
    //     return Math.min(...zahlen);
    // }
}
namespace Aufgabe_B {
    // console.log(isEven(50));                //Für zahl = -1 würde das Programm aktuell keine Lösung geben, da -1+(-2)=-3 und dies nicht von den if statements gedeckt wird :) solltre -1 auch gehen müsste man im falle von negativem Vorzeichen *(-1) Rechnen um ein Korektes Ergebniss zu erhalten.
